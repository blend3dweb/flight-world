...
const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: false, preserveDrawingBuffer:true, logarithmicDepthBuffer:true, powerPreference:'high-performance' });
...
const seaUniforms={time:{value:0},sun:{value:sunDir},eye:{value:new THREE.Vector3()},islands:{value:islands.map(a=>new THREE.Vector4(a[0],a[1],a[2],a[3]))},islandHeights:{value:islands.map(a=>a[4])},environmentMap:{value:atmosphere.cubeTarget.texture},daylight:{value:1},rainAmount:{value:0},snowAmount:{value:0},wind:{value:1},fogColor:{value:atmosphere.fogColor},fogDensity:{value:.000032},moon:{value:atmosphere.moonDir}};
const ocean=new THREE.Mesh(new THREE.PlaneGeometry(150000,150000),new THREE.ShaderMaterial({
  uniforms:seaUniforms,
  vertexShader:`
    #include <common>
    #include <logdepthbuf_pars_vertex>
    varying vec3 wp;void main(){wp=(modelMatrix*vec4(position,1.)).xyz;gl_Position=projectionMatrix*viewMatrix*vec4(wp,1.);
    #include <logdepthbuf_vertex>
    }`,
  fragmentShader:`
    #include <logdepthbuf_pars_fragment>
    varying vec3 wp;uniform float time;uniform vec3 sun;uniform vec3 eye;uniform vec4 islands[12];uniform float islandHeights[12];
    uniform samplerCube environmentMap;uniform float daylight;uniform float rainAmount;uniform float snowAmount;uniform float wind;uniform vec3 fogColor;uniform float fogDensity;uniform vec3 moon;
    float hash(vec2 p){vec3 q=fract(vec3(p.xyx)*.1031);q+=dot(q,q.yzx+33.33);return fract((q.x+q.y)*q.z);}
    float noise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);return mix(mix(hash(i),hash(i+vec2(1,0)),f.x),mix(hash(i+vec2(0,1)),hash(i+vec2(1,1)),f.x),f.y);}
    // Rotated overlapping wave bands avoid a visible square noise lattice.
    float waterHeight(vec2 p){mat2 r=mat2(.8,-.6,.6,.8);return noise(p)*.50+noise(r*p*1.83+vec2(5.1,-2.7))*.30+noise(r*p*3.71+vec2(-9.2,4.6))*.20;}
    float terrainNoise(vec2 p){return sin(p.x*.0021+cos(p.y*.0031)*1.8)*cos(p.y*.0027)+.45*sin(p.x*.0093+p.y*.0061)+.2*cos(p.x*.025-p.y*.019);}
    void main(){
      #include <logdepthbuf_fragment>
      vec2 p=wp.xz;float bed=-10000.;
      for(int i=0;i<12;i++){
        vec4 a=islands[i];float edge=1.-length((p-a.xy)/a.zw)+terrainNoise(p)*.085;
        float h=edge<0.?-3.+edge*150.:pow(edge,1.65)*islandHeights[i]*(.8+.22*terrainNoise(p*.9))+min(edge*95.,8.)-3.;
        ${URBAN_GLSL}
        bed=max(bed,h);
      }
      float depth=max(-bed,0.);float shallows=exp(-depth*.048);float waterVariation=noise(p*.004)*.6+noise(p*.014)*.25;
      vec3 deep=vec3(.004,.068,.10),shallow=vec3(.025,.31,.255);
      vec3 color=mix(deep,shallow,shallows*.84+waterVariation*.16);
      color=mix(color,vec3(.20,.45,.35),exp(-depth*.20)*.50);
      color*=mix(.045,1.,daylight)*(1.-rainAmount*.33);
      vec2 q=p*.13+vec2(time*.29,time*.16)*wind;float waves=waterHeight(q);
      float footprint=max(length(dFdx(q)),length(dFdy(q)));
      float detailFade=(1.-smoothstep(600.,6000.,distance(eye,wp)))*(1.-smoothstep(.035,.25,footprint));
      float nx=(waterHeight(q+vec2(.16,0.))-waterHeight(q-vec2(.16,0.)))*.55;
      float nz=(waterHeight(q+vec2(0.,.16))-waterHeight(q-vec2(0.,.16)))*.55;
      vec2 swell=p*.017+vec2(time*.12,-time*.09);
      float swellFade=(1.-smoothstep(.12,.65,max(length(dFdx(swell)),length(dFdy(swell)))))*(1.-smoothstep(1500.,9000.,distance(eye,wp)));
      float broadX=(waterHeight(swell+vec2(.1,0.))-waterHeight(swell-vec2(.1,0.)))*.06*swellFade;
      float broadZ=(waterHeight(swell+vec2(0.,.1))-waterHeight(swell-vec2(0.,.1)))*.06*swellFade;
      vec3 n=normalize(vec3((nx*detailFade+broadX)*wind,1.,(nz*detailFade+broadZ)*wind));
      vec3 v=normalize(eye-wp);float fres=.035+.965*pow(1.-max(dot(n,v),0.),5.);
      vec3 reflected=reflect(-v,n);reflected.y=max(reflected.y,.006);reflected=normalize(reflected);
      vec3 skyReflection=textureCube(environmentMap,reflected,2.4+rainAmount*1.6).rgb;
      color=mix(color,skyReflection,clamp(fres+.08,0.,1.));
      float sunVisible=smoothstep(-.01,.13,sun.y)*(1.-rainAmount*.93)*(1.-snowAmount*.84);
      float spec=pow(max(dot(reflect(-sun,n),v),0.),180.);color+=vec3(1.,.75,.40)*spec*sunVisible*.7;
      float moonSpec=pow(max(dot(reflect(-moon,n),v),0.),120.);color+=vec3(.12,.18,.3)*moonSpec*(1.-daylight);
      float fade=exp(-pow(distance(eye,wp)*fogDensity,2.));color+=(waves-.7)*.012*detailFade*mix(.05,1.,daylight);
      float foamWidth=max(fwidth(depth)*1.5,.3);
      float foam=(1.-smoothstep(.3,.9+foamWidth,abs(depth-(1.1+.28*sin(time*1.4+p.x*.008)))))*smoothstep(0.,.45,depth);
      color=mix(color,vec3(.60,.67,.60)*mix(.03,1.,daylight),foam*.36);
      color=mix(fogColor,color,fade);gl_FragColor=vec4(max(color,vec3(0.)),1.);
...
terrainMaterial.onBeforeCompile=shader=>{
  ...
  shader.fragmentShader='varying vec3 terrainWorld;\n'+shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
    float detail=sin(terrainWorld.x*.16)*sin(terrainWorld.z*.18)*.045+sin(terrainWorld.x*.51+sin(terrainWorld.z*.21))*cos(terrainWorld.z*.39)*.035;
    diffuseColor.rgb *= .95+detail;`);
};
for(const a of islands){
  ...
  geo.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));geo.computeVertexNormals();scene.add(new THREE.Mesh(geo,terrainMaterial));
}
...
const forest=new THREE.InstancedMesh(new THREE.IcosahedronGeometry(1,0),new THREE.MeshStandardMaterial({color:'#44673c',roughness:1}),forestPoints.length);
const dummy=new THREE.Object3D(), tint=new THREE.Color();
forestPoints.forEach(([x,y,z,s],i)=>{dummy.position.set(x,y+s*.38,z);dummy.scale.set(s*.8,s*.67,s*.85);dummy.rotation.set(random(),random()*6,random());dummy.updateMatrix();forest.setMatrixAt(i,dummy.matrix);tint.setHSL(.25+random()*.08,.25+random()*.14,.17+random()*.12);forest.setColorAt(i,tint);});
scene.add(forest);
...
function draw(){
  ...
  infrastructure.update(atmosphere.daylight,weather.rain);
