...
window.flight.captureShot=async({eye,target})=>{cockpit.visible=aircraft.visible=false;camera.position.set(...eye);camera.lookAt(...target);water.update?.(camera);atmosphere.apply(camera);seaUniforms.eye.value.copy(camera.position);renderer.render(scene,camera);await renderer.backend.device.queue.onSubmittedWorkDone();return renderer.domElement.toDataURL();};
if(offlineRender)window.flight.inspect=()=>({scene,camera,renderer,aircraft,cockpit,atmosphere,water,city,roofMat,windowTex,ocean,ground,islandHeight,islands,infrastructure,forest,forestPoints,reservedLand,PADS});
