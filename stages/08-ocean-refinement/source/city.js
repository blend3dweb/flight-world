...
export function createCity(){
 ...
 function update(daylight,rain,camera){if(camera)for(const data of batches.values()){const mesh=data.mesh,bounds=mesh.boundingSphere;mesh.visible=!bounds||bounds.center.distanceTo(camera.position)<6000+bounds.radius;}for(const f of facades)f.emissiveIntensity=(1-daylight)*1.8;glow.emissiveIntensity=(1-daylight)*3;runwayLight.emissiveIntensity=(1-daylight)*4;m.asphalt.roughness=.92-rain*.63;m.concrete.roughness=.88-rain*.33;}
