...
export function createAtmosphere({renderer,scene,sunlight,hemisphere,sunDir}){
  ...
  const uniforms={rayleigh:sky.rayleigh,turbidity:sky.turbidity,mieCoefficient:sky.mieCoefficient,mieDirectionalG:sky.mieDirectionalG,sunPosition:sky.sunPosition,daylight:uniform(1),cloudCover:sky.cloudCoverage,rainAmount:uniform(0),snowAmount:uniform(0),cloudOrigin:uniform(new THREE.Vector2()),moonDirection:uniform(new THREE.Vector3())};
  ...
  sky.material.colorNode=skyNight({base:vec3(sky.material.colorNode),d:positionLocal.normalize(),sun:uniforms.sunPosition.normalize(),moon:uniforms.moonDirection,day:uniforms.daylight,storm:uniforms.rainAmount.max(uniforms.snowAmount.mul(.8))});
