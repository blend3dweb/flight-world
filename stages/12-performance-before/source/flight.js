...
let W=1600,H=900;
const qualities={low:{scale:.65,reflection:.3},balanced:{scale:.82,reflection:.4},high:{scale:1,reflection:.5}};
let quality=renderer.backend.device.adapterInfo?.vendor==='nvidia'?'high':'balanced';
function setQuality(value){if(!qualities[value])return;quality=value;water.planar.reflector.resolutionScale=qualities[value].reflection;document.querySelector('#quality').value=value;resize();}
water.planar.reflector.resolutionScale=qualities[quality].reflection;
