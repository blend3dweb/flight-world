...
const terrainWetness=uniform(0),terrainMaterial = new THREE.MeshStandardNodeMaterial({roughness:1});
terrainMaterial.colorNode=attribute('color','vec3').mul(sin(positionWorld.x.mul(.16)).mul(sin(positionWorld.z.mul(.18))).mul(.045).add(.95)).mul(terrainWetness.mul(-.18).add(1));
terrainMaterial.roughnessNode=terrainWetness.mul(-.42).add(.98);
for(const a of islands){
  ...
  for(let i=0;i<p.count;i++){
    ...
    // A warm dry beach and darker wet margin share the actual shoreline height.
    const sand=1-THREE.MathUtils.smoothstep(h,3,12),wet=lerp(.60,1,THREE.MathUtils.smoothstep(h,.15,2));
    c.r=lerp(c.r,(.27+n*.03)*wet,sand);c.g=lerp(c.g,(.22+n*.027)*wet,sand);c.b=lerp(c.b,(.125+n*.015)*wet,sand);
...
let W=1600,H=900;
const qualities={low:{scale:.65,reflection:.25,reflectionEvery:4,shadowEvery:4},balanced:{scale:.82,reflection:.34,reflectionEvery:3,shadowEvery:3},high:{scale:1,reflection:.42,reflectionEvery:3,shadowEvery:3}};
let quality=renderer.backend.device.adapterInfo?.vendor==='nvidia'?'high':'balanced';
function setQuality(value){if(!qualities[value])return;quality=value;water.configure({resolutionScale:qualities[value].reflection,interval:qualities[value].reflectionEvery});document.querySelector('#quality').value=value;resize();}
water.configure({resolutionScale:qualities[quality].reflection,interval:qualities[quality].reflectionEvery});
document.querySelector('#quality').value=quality;document.querySelector('#quality').onchange=e=>setQuality(e.target.value);
function resize(){W=canvas.width=Math.round(innerWidth* Math.min(devicePixelRatio,1.5));H=canvas.height=Math.round(innerHeight*Math.min(devicePixelRatio,1.5));renderer.setSize(Math.round(W*qualities[quality].scale),Math.round(H*qualities[quality].scale),false);camera.aspect=W/H;camera.updateProjectionMatrix();draw();}
...
function draw(){
  ...
  vegetation.update(camera,state.time,atmosphere.wind);
  atmosphere.apply(camera);
  sunlight.shadow.needsUpdate=sunlight.intensity>.08&&(renderFrame===1||renderFrame%qualities[quality].shadowEvery===0);
  const weather=atmosphere.getState();
  terrainWetness.value=weather.rain;
  ...
  water.configure({interval:qualities[quality].reflectionEvery+(viewMode==='external'&&orbit.auto?1:0)});
  water.update?.(camera,{daylight:atmosphere.daylight});
...
document.querySelector('#record').onclick=()=>{
  if(recording)return;
  if(!canvas.captureStream||typeof MediaRecorder==='undefined'){toast('Этот браузер не поддерживает запись видео.');return;}
  ...
  const stream=composeFrame().captureStream(30);let rec;
