...
export function createCity(){
 ...
 const windowDensity={value:.275}; // Previously .55: halve occupancy, retain lamp brightness.
 ...
 const facades=[0,1,2,3].map(i=>{
   const material=new THREE.MeshStandardMaterial({color:['#77a5ad','#b9d2cf','#dfd9c4','#6d9da7'][i],roughness:i===2?.65:.29,metalness:i===2?.08:.42,emissive:'#ffca80',emissiveIntensity:0});
   material.onBeforeCompile=s=>{
    s.uniforms.windowDensity=windowDensity;
    s.vertexShader='varying vec2 cityFacade; varying vec2 citySeed;\n'+s.vertexShader.replace('#include <begin_vertex>',`#include <begin_vertex>
      vec3 cs=vec3(1.);citySeed=vec2(0.);
      #ifdef USE_INSTANCING
      cs=vec3(length(instanceMatrix[0].xyz),length(instanceMatrix[1].xyz),length(instanceMatrix[2].xyz));citySeed=instanceMatrix[3].xz;
      #endif
      cityFacade=vec2(uv.x*${i===3?'cs.x*6.283185':'(abs(normal.x)>.5?cs.z:cs.x)'},uv.y*cs.y);
    `);
    s.fragmentShader='uniform float windowDensity; varying vec2 cityFacade; varying vec2 citySeed;\n'+s.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
      vec2 grid=cityFacade/vec2(2.9,3.8);vec2 cell=fract(grid);vec2 aa=fwidth(grid)*1.2;
      float win=smoothstep(.10-aa.x,.10+aa.x,cell.x)*(1.-smoothstep(.87-aa.x,.87+aa.x,cell.x))*smoothstep(.20-aa.y,.20+aa.y,cell.y)*(1.-smoothstep(.91-aa.y,.91+aa.y,cell.y));
      diffuseColor.rgb*=mix(vec3(1.18),vec3(.31,.53,.62),win);
      float occupied=step(1.-windowDensity,fract(sin(dot(floor(grid)+floor(citySeed),vec2(12.9898,78.233)))*43758.5453));
    `).replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\ntotalEmissiveRadiance*=win*occupied;');
   };
   material.customProgramCacheKey=()=>`city-facade-v5-${i}`;
   return material;
 });
 ...
 for(const [name,data] of batches){
   const mesh=new THREE.InstancedMesh(data.geometry,data.material,data.items.length);mesh.name=name;
