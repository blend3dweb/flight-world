...
export function createOcean({renderer,scene,atmosphere,sunDir,ground}){
 ...
 const planar=reflector({resolutionScale:.42,bounces:false,samples:1,generateMipmaps:true});
 // Keep Three.js in charge of the reflector render so its camera, clipping and
 // render state stay valid. A small gate below decides which live frames need
 // a new cached planar texture.
 ...
 let frame=0,reflectionUpdates=0,reflectionEvery=3,baseResolutionScale=.42,updateRequested=true,expectedCamera=null;
 let observedCamera=null,lastCameraQuaternion=new T.Quaternion(),angularDelta=0,turnHold=0,turning=false;
 const updateReflector=planar.reflector.updateBefore.bind(planar.reflector);
 planar.reflector.updateBefore=nodeFrame=>{
  // Environment, shadow and nested reflection passes use temporary cameras.
  // Keep the queued update for the camera whose image reaches the screen.
  if(nodeFrame.camera!==expectedCamera)return false;
  if(!updateRequested&&planar.reflector.hasOutput)return false;
  const result=updateReflector(nodeFrame);updateRequested=false;reflectionUpdates++;return result;
 };
 function update(camera,{force=false,daylight=1,adaptiveRotation=false}={}){
  expectedCamera=camera;ocean.position.set(camera.position.x,0,camera.position.z);frame++;
  if(observedCamera===camera)angularDelta=lastCameraQuaternion.angleTo(camera.quaternion);else{observedCamera=camera;angularDelta=0;}
  lastCameraQuaternion.copy(camera.quaternion);
  if(adaptiveRotation&&angularDelta>.0012)turnHold=8;else turnHold=Math.max(0,turnHold-1);
  turning=adaptiveRotation&&turnHold>0;
  // A cached screen-space reflection is noticeable while the cockpit rotates.
  // Refresh it every turning frame, using a smaller target to cap the cost.
  planar.reflector.resolutionScale=baseResolutionScale*(.76+.24*daylight)*(turning?.66:1);
  // Offset the reflection update from the shadow cadence so both extra passes
  // normally do not land in the same frame.
  if(force||turning||!planar.reflector.hasOutput||frame%reflectionEvery===1)updateRequested=true;
 }
 return {ocean,uniforms,planar,heightMap,coastMap,update,configure({resolutionScale,interval}){if(Number.isFinite(resolutionScale))baseResolutionScale=resolutionScale;if(Number.isFinite(interval))reflectionEvery=Math.max(1,Math.round(interval));},getState:()=>({reflectionUpdates,reflectionEvery,resolutionScale:planar.reflector.resolutionScale,baseResolutionScale,turning,angularDelta,turnHold}),dispose(){geo.dispose();mat.dispose();planar.dispose();heightMap.dispose();coastMap.dispose();}};
}
