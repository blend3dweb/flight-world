...
import {createDetailBuilders} from './city-details.js';
...
export function createCity(){
 ...
 const m={asphalt:mat('#303d42'),concrete:mat('#b5c1b9'),white:mat('#eee8d6'),sand:mat('#cebfa0'),grass:mat('#517b48'),wood:mat('#a37143'),metal:mat('#465c5b',.4,.55),rubber:mat('#17282b'),trunk:mat('#766141'),leaf:mat('#417d53'),glass:mat('#396d79',.21,.55),roof:mat('#adc2c4',.66,.25),accent:mat('#c7774f',.62,.08),stone:mat('#786f65',.82),yellow:mat('#eec15d'),red:mat('#c96852'),water:mat('#3298a8',.2,.45)};
 ...
 function b(name,material,x,y,z,w,h,d,rot=0,color){put(name,box,material,x,y,z,w,h,d,rot,color);if(Array.isArray(material)&&h>10&&rot===0)details.facade(x,y,z,w,h,d);}
 const details=createDetailBuilders({b,put,box,cylinder,m});
 const stats={blocks:0,buildings:0,entrances:0,benches:0,palms:0,cars:0,crosswalks:0,streetlights:0,busStops:0,planters:0,bikeRacks:0,bollards:0,storefronts:0,balconies:0,landmarks:0,runways:1,gates:4};
 ...
 function planter(x,y,z,a=0){stats.planters++;part('Street planters',m.stone,x,y,z,a,0,.35,0,2.2,.7,1.1);for(const dx of [-.65,0,.65])part('Planter foliage',m.leaf,x,y,z,a,dx,1.05,0,.72,.9,.72);}
 function bikeRack(x,y,z,a=0){stats.bikeRacks++;for(const dx of [-.75,0,.75]){part('Bike rack posts',m.metal,x,y,z,a,dx,.48,0,.08,.96,.08);part('Bike rack rails',m.metal,x,y,z,a,dx,.91,0,.08,.08,.72);}}
 function bollards(x,y,z,a=0){for(let i=-2;i<=2;i++){stats.bollards++;part('Street bollards',m.metal,x,y,z,a,i*1.8,.48,0,.18,.96,.18);part('Bollard reflectors',m.white,x,y,z,a,i*1.8,.73,0,.20,.10,.20);}}
 function busStop(x,y,z,a=0){stats.busStops++;part('Bus stop glass',m.glass,x,y,z,a,0,1.7,-.55,7,3.4,.10);for(const dx of [-3.45,3.45])part('Bus stop frames',m.metal,x,y,z,a,dx,1.7,-.55,.13,3.4,.18);part('Bus stop canopy',m.accent,x,y,z,a,0,3.55,.45,7.8,.24,2.2);part('Bus stop seats',m.wood,x,y,z,a,0,.72,-.08,4.2,.18,.65);part('Bus stop sign',m.yellow,x,y,z,a,3.8,2.45,.1,.72,1.25,.10);}
 ...
 function building(x,z,w,d,h,type=0,front=0,angle=0,kind=0,block){
   ...
   if(kind===0)details.roof(x,y+h+(h>70?10.6:.5),z,w*(h>70?.70:1),d*(h>70?.70:1));
   if(kind===1)details.roof(x,y+h+.8,z,w*.5,d*.5);
   if(kind===2)for(const side of [-1,1])details.roof(x+side*w*.34,y+h+.5,z,w*.28,d*.90);
   if(kind===3)details.roof(x,y+h+.3,z-d*.24,w*.9,d*.44);
   if(kind===5)for(const side of [-1,1])details.roof(x+side*w*.3,y+6+h*(side>0?.77:1)+.5,z,w*.3,d*.8);
   // Street fronts have real depth: glazing sits behind frames, with canopies
   // and occasional balconies instead of relying on the facade shader alone.
   const shopWidth=Math.max(10,w*.76),shopCount=Math.max(3,Math.floor(shopWidth/6));stats.storefronts++;
   part('Storefront recesses',m.stone,x,y,z,front,0,2.35,d/2+1.54,shopWidth,4.35,.34);
   part('Storefront glazing',m.glass,x,y,z,front,0,2.35,d/2+1.76,shopWidth-.8,3.75,.10);
   for(let i=0;i<=shopCount;i++)part('Storefront mullions',m.metal,x,y,z,front,-shopWidth/2+i*shopWidth/shopCount,2.35,d/2+1.84,.10,3.85,.11);
   part('Storefront awnings',kind%2?m.accent:m.roof,x,y,z,front,0,4.65,d/2+2.35,shopWidth+.8,.28,1.85);
   if([2,3,4].includes(kind))for(let level=8;level<h-4;level+=7.6){stats.balconies++;part('Balcony slabs',m.concrete,x,y,z,front,0,level,d/2+1.12,w*.68,.22,2.4);part('Balcony glass',m.glass,x,y,z,front,0,level+.9,d/2+2.25,w*.68,1.6,.10);}
 ...
 for(let ix=0;ix<6;ix++)for(let iz=0;iz<5;iz++){
   ...
   for(let e=0;e<4;e++){
     ...
     if((ix+iz+e)%3===0)busStop(mx+dx*.17,Y+.4,mz+dz*.17,angle-Math.PI/2);else bikeRack(mx-dx*.16,Y+.4,mz-dz*.16,angle-Math.PI/2);
     for(const t of [.28,.72])planter(a[0]+dx*t+nx*3,Y+.4,a[1]+dz*t+nz*3,angle-Math.PI/2);
   }
 }
 // A low civic building gives the park a public use and a distinct silhouette.
 stats.landmarks++;const civicX=-2025,civicZ=-1535;
 b('Civic pavilion plinth',m.stone,civicX,Y+1,civicZ,55,2,35,.10);
 b('Civic pavilion west wing',m.white,civicX-12,Y+6,civicZ,34,10,24,.22);
 b('Civic pavilion east wing',m.accent,civicX+13,Y+4.5,civicZ-2,30,7,27,-.18);
 b('Civic pavilion atrium',m.glass,civicX,Y+7,civicZ+4,18,12,24,.04);
 b('Civic pavilion canopy',m.white,civicX,Y+13.3,civicZ+2,48,.8,30,-.08);
 for(let i=-4;i<=4;i++)b('Civic pavilion fins',i%2?m.accent:m.white,civicX+i*4.2,Y+7,civicZ+17,.45,10,.65,-.08);
 b('Civic pavilion steps',m.concrete,civicX,Y+.55,civicZ+25,30,.3,10);bollards(civicX,Y+.4,civicZ+31,Math.PI/2);
 ...
 for(let z=A.z-160;z<=A.z+160;z+=32){b('Terminal canopy columns',m.white,2053,AY+4.2,z,.6,8.4,.6);bollards(2047,AY+.3,z,0);}
 for(let z=A.z-150;z<=A.z+150;z+=60)busStop(2038,AY+.35,z,0);
 ...
 for(const [name,data] of batches){
   ...
   mesh.computeBoundingSphere();mesh.castShadow=data.items.some(v=>v.h>3);mesh.receiveShadow=true;group.add(mesh);data.mesh=mesh;
 }
