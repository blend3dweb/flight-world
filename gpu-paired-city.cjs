...
const out=path.join(__dirname,'stages/10-city-refinement');
if(fs.existsSync(path.join(out,'complete.json')))throw Error('Completed stage is protected.');

(async()=>{let browser;const errors=[],samples=[];try{
 for(let round=0;round<3;round++)for(const variant of (round%2?['after','before']:['before','after'])){
  ...
  await page.route('**/webgpu/city.js',route=>variant==='before'?route.fulfill({path:path.join(__dirname,'stages/10-city-before/source/city.js'),contentType:'text/javascript'}):route.continue());
  ...
  const device=await page.evaluate(()=>{flight.startFlight('city');flight.setEnvironment({hour:10,cycle:false,weather:'sun',autoWeather:false},{immediate:true});flight.setView('external');flight.setOrbit({auto:true,distance:30});const r=flight.inspect().renderer;return {vendor:r.backend.device.adapterInfo.vendor,width:r.domElement.width,height:r.domElement.height,quality:flight.getState().quality};});
  assert.deepEqual(device,{vendor:'nvidia',width:1920,height:1080,quality:'high'});
  const timing=await page.evaluate(async()=>{await new Promise(r=>setTimeout(r,8000));await new Promise(resolve=>{let frames=0;function warm(){if(++frames>=30)resolve();else requestAnimationFrame(warm);}requestAnimationFrame(warm);});const times=[];await new Promise(resolve=>{const start=performance.now();let last=start;function tick(now){times.push(now-last);last=now;if(now-start<8000)requestAnimationFrame(tick);else resolve();}requestAnimationFrame(tick);});times.shift();times.sort((a,b)=>a-b);return {frames:times.length,averageFps:1000/(times.reduce((a,b)=>a+b,0)/times.length),medianMs:times[Math.floor(times.length*.5)],p95Ms:times[Math.floor(times.length*.95)]};});
  samples.push({round:round+1,variant,...timing});console.log(JSON.stringify(samples.at(-1)));await browser.close();browser=null;
 }
 const median=values=>values.sort((a,b)=>a-b)[Math.floor(values.length/2)],summary={};for(const variant of ['before','after']){const rows=samples.filter(s=>s.variant===variant);summary[variant]={averageFps:median(rows.map(s=>s.averageFps)),medianMs:median(rows.map(s=>s.medianMs)),p95Ms:median(rows.map(s=>s.p95Ms))};}summary.fpsChangePercent=(summary.after.averageFps/summary.before.averageFps-1)*100;summary.p95ChangePercent=(summary.after.p95Ms/summary.before.p95Ms-1)*100;assert.deepEqual(errors,[]);fs.writeFileSync(path.join(out,'paired-city-fresh.json'),JSON.stringify({date:new Date().toISOString(),mode:'Visible Chrome, NVIDIA, 1920x1080 high, city orbit, 8s warmup plus 30 presented frames + 8s measurement, alternating A/B order, fresh browser for each sample',samples,summary,errors},null,2));console.log(JSON.stringify(summary,null,2));
